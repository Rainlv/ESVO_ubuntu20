...
void TimeSurface::eventsCallback(const prophesee_event_msgs::EventArray::ConstPtr& msg)
{
  std::lock_guard<std::mutex> lock(data_mutex_);
  ...
  dvs_msgs::EventArray dvs_evt_arr;
  for (const prophesee_event_msgs::Event& e : msg->events)
  {
    dvs_msgs::Event dvs_evt;
    dvs_evt.x = e.x;
    dvs_evt.y = e.y;
    dvs_evt.ts = e.ts;
    dvs_evt.polarity = e.polarity;
    dvs_evt_arr.events.push_back(dvs_evt);
  }

  const dvs_msgs::EventArray::ConstPtr dvs_evt_arr_ptr = boost::make_shared<dvs_msgs::EventArray>(dvs_evt_arr);

  for(const dvs_msgs::Event& e : dvs_evt_arr_ptr->events)
  {
    events_.push_back(e);
